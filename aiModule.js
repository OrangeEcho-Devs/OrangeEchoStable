module.exports = {
	...
	execute(input, author, returnFunction){
		...
		if(input == `you can't get the sus emojis from Daniel's bot emoji server?`){
			returnFunction(`...`)
			setTimeout(function(){
				returnFunction(`Actually I can`)
			}, 2000)
			setTimeout(function(){
				returnFunction(`Orange Group #5362 stole it from the Freshman Devs server and added it to this server`)
			}, 2000)
			setTimeout(function(){
				returnFunction(`See <:sus:721329850756890624>`)
			})
		} else  
		if(input == `no`){
			returnFunction(`u`)
