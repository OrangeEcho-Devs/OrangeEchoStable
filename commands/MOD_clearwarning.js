      ...
      if(!message.mentions.members.first()) {
        ...
        if (message.author.id == user){
         ...
         //const {ModeratorRoleID} = db.fetch(`ModeratorRoleID_${message.guild.id}`)
          ...
          //}
          if (fs.existsSync('./logs/' + user + '-warnings.log')
              &&
              fs.existsSync('./logs/' + user + '-modwarnings.log')){
    
            fs.unlink('./logs/' + user + '-warnings.log',function(error){
              if(error){
                console.log(error);
            ...
            fs.unlink('./logs/' + user + '-modwarnings.log',function(error){
              if(error){
                console.log(error);
                respond('Error', 'Something went wrong.\n'+error+`\nMessage: ${message}\nArgs: ${args}\n`, message.channel)
                errorlog(error)
            ...
            respond('✏️','<@'+user + '> had their punishment log cleared.', message.channel)
